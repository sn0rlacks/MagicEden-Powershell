<#
.SYNOPSIS
    Get received offers for a token / NFT.
...
function Get-Offers {
    ...
    switch ($Network) {
        "Mainnet" {$api = "$MainnetAPI/tokens/$MintAddress/offers_received?offset=$Offset&limit=$Limit"}
        "Devnet" {$api = "$DevnetAPI/tokens/$MintAddress/offers_received?offset=$Offset&limit=$Limit"}
    }
    try {
